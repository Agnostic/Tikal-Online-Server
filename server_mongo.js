...
var app = require("express").createServer(),
	server = app.listen(1213);
	nowjs = require("now"),
	clientes = nowjs.initialize(server),
	db = require("mongous").Mongous;
/*
// DB
var col_personajes = "tikalonline.personajes",
	col_usuarios = "tikalonline.usuarios";

var players = {},
	fecha = new Date(),
	porcentajeExpFromMobs = {};
...
nowjs.on('disconnect', function() {
  ...
  for(var i in players) {
    if(i == this.user.clientId) {
      ...
      db(col_personajes).save( { _id: ObjectId(player_id[this.user.clientId].id) }, { "$set" : { x: this.now.player.x, y: this.now.player.y } } );
      ...
      db(col_personajes).save( { _id: ObjectId(player_id[this.user.clientId].id) }, { "$set" : { inventario: this.now.player.inventario } } );
...
clientes.now.loginUsuario = function(data){
	...
	db(col_usuarios).find({ usuario: data.u }, function(results){
            if(typeof(results[0]) == 'undefined'){
					loginUsuario.now.loggedOn({ l: 0, m: "The user does not exist" });
            } else {
				db(col_usuarios).find({ usuario: data.u, password: data.p }, function(results){
	                if(typeof(results[0]) == 'undefined'){
	                    loginUsuario.now.loggedOn({ l: 0, m: "Incorrect password, try again." });
	                } else {
			        	db(col_usuarios).find({ cuenta_id: ObjectId(results[0]._id) }, function(results){
				        	if(typeof(results[0]) != 'undefined'){
